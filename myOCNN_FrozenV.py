...
def ocfnn_obj(theta, X, nu, V, D, K, g, dG):
...
def ocfnn_grad(theta, X, nu, V, D, K, g, dG):
    ...
    term2 = np.concatenate(( np.zeros((w.size,)),
                             0*np.zeros((V.flatten().size,)), #0*V.flatten(),
                             np.zeros((bH.size,)),
                             np.zeros((1,)) ))

    term3 = np.concatenate(( 1/nu * np.mean(deriv[:,np.newaxis] * (-hiddenScore(X, V, bH, g)), axis = 0),
                             0*1/nu * np.mean((deriv[:,np.newaxis] * (dG(X.dot(V) + bH) * -w)).reshape((N, 1, K)) * X.reshape((N, D, 1)), axis = 0).flatten(),
...
class MyOCNN_FrozenV:

    def __init__(self, nu, V, K, g, dG):
    ...
    def fit(self, XTr, theta0):
        ...
        print('Gradient error: %s' % check_grad(ocfnn_obj, ocfnn_grad, theta0, XTr, self.nu, self.V, D, K, self.g, self.dG))
        resFNN = minimize(ocfnn_obj, theta0, 
                          jac     = ocfnn_grad, 
                          args    = (XTr, self.nu, self.V, D, K, self.g, self.dG),
                          method  = 'L-BFGS-B',                
                          options = {'gtol': 1e-16, 'disp': True, 'maxiter' : 50000, 'maxfun' : 10000})
        print('Gradient error: %s' % check_grad(ocfnn_obj, ocfnn_grad, resFNN.x, XTr, self.nu, self.V, D, K, self.g, self.dG))
